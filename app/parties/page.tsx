...
function estimateShare(currentCount: number, capacity: number) {
  const estimatedTotalFare = 12000;
  const headcount = Math.min(Math.max(currentCount + 1, 2), capacity);
  return Math.ceil(estimatedTotalFare / headcount / 100) * 100;
}
      ...
      <Card className="p-5">
        <div className="space-y-4">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl font-bold text-slateBlue">
              {q ? `${q}에서 출발` : "어디서 출발하나요?"}
            </h1>
          </div>
          ...
          {chooser || !q ? (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <Link href="/parties?q=부천역" className={`${buttonStyles("secondary", true)} text-sm`}>
                  부천역
                </Link>
                <Link href="/parties?q=송내역" className={`${buttonStyles("secondary", true)} text-sm`}>
                  송내역
                </Link>
                <Link href="/parties?q=온수역" className={`${buttonStyles("secondary", true)} text-sm`}>
                  온수역
                </Link>
                <Link href="/parties?q=역곡역" className={`${buttonStyles("secondary", true)} text-sm`}>
                  역곡역
                </Link>
              </div>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <Link href="/parties?chooser=1" className="text-sm font-semibold text-brand-700 underline underline-offset-4">
                출발지 바꾸기
              </Link>
            </div>
          )}
        ...
        activeParties.length > 0 ? (
          <div className="space-y-3">
            {activeParties.map((party) => {
              ...
              const estimatedShare = estimateShare(party.joinedCount, party.capacity);
                ...
                <Card key={party.id} className="p-5">
                  <div className="space-y-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-semibold ${
                          immediate ? "bg-brand-100 text-brand-700" : "bg-slate-100 text-slate-600"
                        }`}
                      >
                        {immediate ? "바로 출발" : "예약"}
                      </span>
                    ...
                    <div className="space-y-1 text-sm text-slate-600">
                      <p>
                        현재 인원 / 최대 인원: {party.joinedCount}/{party.capacity}명
                      </p>
          ...
          <EmptyState
            ...
            description="바로 참여할 수 있는 팟이 없다면 직접 모집을 시작해보세요."
